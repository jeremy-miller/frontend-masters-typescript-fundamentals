...
function mapDict<T, S>(
  inputDict: Dict<T>,
  mapFunc: (originalValue: T, key: string) => S
): Dict<S> {
  const outputDict: Dict<S> = {};
  for (const k of Object.keys(inputDict)) {
    const val: T = inputDict[k];
    outputDict[k] = mapFunc(val, k);
  }
  return outputDict;
}
...
function filterDict<T>(
  inputDict: Dict<T>,
  filterFunc: (value: T, key: string) => boolean
): Dict<T> {
...
function reduceDict<T, S>(
  inputDict: Dict<T>,
  reduceFunc: (currentVal: S, dictItem: T, key: String) => S,
  initialVal: S
): S {
  let value = initialVal;
  for (const k of Object.keys(inputDict)) {
    const val = inputDict[k];
    value = reduceFunc(value, val, k);
  }
  return value;
}
